...
for (i = 0; i < obj.length; i++) {
  if (obj[i].checked) // radio box checked 
		document.editform.wpTextbox1.value += "[[Category:"+obj[i].value+"]]";
	else { // radio box not checked
		// search and remove markups
		console.log('object length: ' + obj.length)
		var pattern = "\[\[Category:" + obj[i].value + "\]\]";
		var myregex = new RegExp(pattern);
		var res = document.editform.wpTextbox1.value.match(myregex);
		document.editform.wpTextbox1.value = document.editform.wpTextbox1.value.replace(pattern,""); //moved
